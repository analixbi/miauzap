...
func resetKillChannel(userID string) {
	killchannelMu.Lock()
	defer killchannelMu.Unlock()
	killchannel[userID] = make(chan bool, 1)
}

func ensureKillChannel(userID string) {
	killchannelMu.Lock()
	defer killchannelMu.Unlock()
	if _, exists := killchannel[userID]; !exists {
		killchannel[userID] = make(chan bool, 1)
	}
}

func getKillChannel(userID string) <-chan bool {
	killchannelMu.RLock()
	defer killchannelMu.RUnlock()
	return killchannel[userID]
}

func signalKillChannel(userID string) {
	killchannelMu.RLock()
	ch := killchannel[userID]
	killchannelMu.RUnlock()
	if ch == nil {
		return
	}
	select {
