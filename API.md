...
2. **Admin Token**: For admin endpoints (/admin/**), use the `Authorization` header with the admin token value (set in MIAUZAP_ADMIN_TOKEN).
...
The following admin-only endpoints are used to manage users in the system. All require the Authorization header with the admin token (MIAUZAP_ADMIN_TOKEN).
...
Example Request:
```
curl -s -X GET -H 'Authorization: {{MIAUZAP_ADMIN_TOKEN}}' http://localhost:8080/admin/users
```
...
Example Request:
```
curl -s -X POST -H 'Authorization: {{MIAUZAP_ADMIN_TOKEN}}' -H 'Content-Type: application/json' --data '{"name":"usuario2","token":"token2","webhook":"https://example.com/webhook2","events":"Message,ReadReceipt"}' http://localhost:8080/admin/users
```
...
Example Request:
```
curl -s -X DELETE -H 'Authorization: {{MIAUZAP_ADMIN_TOKEN}}' http://localhost:8080/admin/users/2
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"number":"5491155554444","text":"Hellow Meow", "id": "90B2F8B13FAC8A9CF6B06E99C7834DC5"}' http://localhost:8080/chat/send/text
```

Example sending a new message with link preview
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"number":"5491155554444","text":"Check my site? https://example.com", "id": "90B2F8B13FAC8A9CF6B06E99C7834DC5","linkPreview": true}' http://localhost:8080/chat/send/text
```

Example replying to some message:

```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"number":"5491155554444","text":"Ditto","ContextInfo":{"StanzaId":"AA3DSE28UDJES3","Participant":"5491155553935@s.whatsapp.net"}}' http://localhost:8080/chat/send/text
```
...
Sends an interactive message with buttons. Buttons can be of type `reply`, `url`, `copy`, or `call`. It automatically uses the newer NativeFlow structure (wrapped for Multi-Device compatibility).
...
```json
{
  ...
  "footer": "Miauzap Official",
  "buttons": [
    { "id": "sales", "text": "Sales", "type": "reply" },
    { "id": "support", "text": "Support", "type": "reply" },
    { "id": "https://analixbi.com.br", "text": "Our Website", "type": "url" },
    { "id": "MIAU20", "text": "Copy Coupon", "type": "copy" },
    { "id": "5511999999999", "text": "Call Us", "type": "call" }
  ]
}
```

```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"number":"1234567890","title":"Support Options","body":"How can we help you today? \ud83d\udc31","footer":"Miauzap Official","buttons":[{"id":"sales","text":"Sales","type":"reply"},{"id":"support","text":"Support","type":"reply"},{"id":"https://analixbi.com.br","text":"Our Website","type":"url"},{"id":"MIAU20","text":"Copy Coupon","type":"copy"},{"id":"5511999999999","text":"Call Us","type":"call"}]}' http://localhost:8080/chat/send/buttons
```
...
Sends an interactive list (menu) message with sections. It uses the NativeFlow `single_select` component ensuring high compatibility.
...
```json
{
  "number": "1234567890",
  "header": "Catalog",
  "body": "Choose one of our products below: 🐱",
  "footer": "Miauzap Official",
  ...
  "sections": [
    {
      "title": "Cat Food",
      "rows": [
        { "id": "premium", "title": "Premium Food", "desc": "Salmon Flavor" },
        { "id": "standard", "title": "Standard Food", "desc": "Chicken Flavor" }
      ]
    },
    {
      "title": "Accessories",
      "rows": [
        { "id": "scratcher", "title": "Tower Scratcher", "desc": "1.5m high" },
        { "id": "toy", "title": "Plush Mouse", "desc": "With Catnip" }
      ]
    }
  ]
}
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"number":"1234567890","header":"Catalog","body":"Choose one of our products below: \ud83d\udc31","footer":"Miauzap Official","button_text":"View Products","sections":[{"title":"Cat Food","rows":[{"id":"premium","title":"Premium Food","desc":"Salmon Flavor"},{"id":"standard","title":"Standard Food","desc":"Chicken Flavor"}]},{"title":"Accessories","rows":[{"id":"scratcher","title":"Tower Scratcher","desc":"1.5m high"},{"id":"toy","title":"Plush Mouse","desc":"With Catnip"}]}]}' http://localhost:8080/chat/send/list
```
...
Sends a carousel message with cards. Cards can contain images, videos, or documents.

Endpoint: _/chat/send/carousel_

Method: **POST**


```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"number":"5491155554444","text":"Check out our products","carousel":[{"image":"https://example.com/product1.jpg","body":"Product 1\nBest seller","buttons":[{"display_text":"Buy Now","url":"https://example.com/buy/1"}]},{"video":"https://example.com/demo.mp4","body":"See it in action","buttons":[{"display_text":"More Info","id":"info_2"}]}]}' http://localhost:8080/chat/send/carousel
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"number":"5491155554444", "audio":"data:audio/ogg;base64,iVBORw0KGgoAAAANSU..."}' http://localhost:8080/chat/send/audio
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"number":"5491155554444","caption":"Look at this", "image":"https://example.com/logo.jpeg"}' http://localhost:8080/chat/send/image
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"number":"5491155554444","fileName":"hola.txt","document":"data:application/octet-stream;base64,aG9sYSBxdWUgdGFsCg=="}' http://localhost:8080/chat/send/document
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"number":"5491155554444","caption":"Look at this", "video":"data:video/mp4;base64,iVBORw0KGgoAAAANSU..."}' http://localhost:8080/chat/send/video
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"group":"120363313346913103@g.us", "header": "Qual o seu pet favorito?", "options": ["Gato 🐱", "Cachorro 🐶", "Passarinho 🐦", "Peixe 🐟"]}' http://localhost:8080/chat/send/poll
```
...
Endpoint: _/chat/send/edit_

Method: **POST**

```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"phone":"5491155554444", "body":"Texto corrigido após enviar 🐱", "id":"MESSAGE_ID_TO_EDIT"}' http://localhost:8080/chat/send/edit
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"phone":"5491155554444", "id":"MESSAGE_ID_TO_DELETE"}' http://localhost:8080/chat/delete
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"phone":"5491155554444", "body":"❤️", "id":"MESSAGE_ID_TO_REACT_TO", "participant": ""}' http://localhost:8080/chat/react
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"jid":"5491155554444@s.whatsapp.net", "archive":true}' http://localhost:8080/chat/archive
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"phone":"5491155554444", "state":"composing"}' http://localhost:8080/chat/presence
```
...
```
curl -X POST -H 'Token: 1234ABCD' -H 'Content-Type: application/json' --data '{"Id":["MESSAGE_ID_1","MESSAGE_ID_2"], "ChatPhone":"5491155554444", "SenderPhone":"5491155554444"}' http://localhost:8080/chat/markread
```
...
Returns the messages history for a specific chat. Due to limitations, limit might not exceed a predefined number set during the server configuration.

Endpoint: _/chat/history_
...
# S3 Storage Integration for Miauzap

## Overview

Miauzap now supports S3-compatible storage for media files, allowing you to store WhatsApp media (images, videos, audio, and documents) in cloud storage services instead of or in addition to base64 encoding in webhooks.
