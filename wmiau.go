...
func updateAndGetUserSubscriptions(mycli *MyClient) ([]string, error) {
	...
	if found2 {
		...
		if err := mycli.db.Get(&currentEvents, mycli.db.Rebind("SELECT events FROM users WHERE id=?"), mycli.userID); err != nil {
...
func (s *server) startClient(userID string, textjid string, token string, subscriptions []string) {
	...
	var proxyURL string
	err = s.db.Get(&proxyURL, s.db.Rebind("SELECT proxy_url FROM users WHERE id=?"), userID)
	if err == nil && proxyURL != "" {
	...
	if client.Store.ID == nil {
		...
		if err != nil {
			...
			for evt := range qrChan {
				if evt.Event == "code" {
					...
					sqlStmt := s.db.Rebind(`UPDATE users SET qrcode=? WHERE id=?`)
					...
					signalKillChannel(userID)
				} else if evt.Event == "success" {
					log.Info().Msg("QR pairing ok!")
					// Clear QR code after pairing
					sqlStmt := s.db.Rebind(`UPDATE users SET qrcode='', connected=1 WHERE id=?`)
					_, err := s.db.Exec(sqlStmt, userID)
					if err != nil {
						log.Error().Err(err).Msg(sqlStmt)
					} else {
		...
		if lastErr != nil {
			...
			sqlStmt := s.db.Rebind(`UPDATE users SET qrcode='', connected=0 WHERE id=?`)
	...
	for {
		stopChannel := getKillChannel(userID)
		select {
		case <-stopChannel:
...
func (mycli *MyClient) syncChatwootMessageReceiptStatus(messageIDs []string, status string) {
	if mycli == nil || mycli.db == nil || len(messageIDs) == 0 || status == "" {
		return
	}

	var cwConfig ChatwootConfig
	err := mycli.db.QueryRow(mycli.db.Rebind(`
	...
	if err != nil || !cwConfig.Enabled || !cwConfig.SendReadReceipts {
		return
	}

	svc := NewChatwootService(mycli.db, cwConfig, mycli.WAClient)
	for _, messageID := range messageIDs {
		if messageID == "" {
			continue
		}
		if err := svc.UpdateMessageStatus(messageID, status); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				log.Debug().Str("waMessageID", messageID).Str("status", status).Msg("Skipping Chatwoot receipt status without outgoing mapping")
			} else {
				log.Warn().Err(err).Str("waMessageID", messageID).Str("status", status).Msg("Failed to sync WhatsApp receipt status to Chatwoot")
			}
			continue
		}
		log.Debug().Str("waMessageID", messageID).Str("status", status).Msg("Synced WhatsApp receipt status to Chatwoot")
	}
}
...
func (mycli *MyClient) myEventHandler(rawEvt interface{}) {
	...
	case *events.Connected, *events.PushNameSetting:
		...
		mycli.ConnectTime = time.Now()

		sqlStmt := mycli.db.Rebind(`UPDATE users SET connected=1 WHERE id=?`)
		...
		// Proactively fetch labels to ensure mappings exist
		go mycli.fetchAndStoreLabels()
	case *events.PairSuccess:
		...
		query := mycli.db.Rebind("SELECT COALESCE(days_to_sync_history, 0) FROM users WHERE id=?")
	...
	case *events.Message:
		...
		dedupKey := fmt.Sprintf("%s:%s:%s", mycli.userID, evt.Info.Chat.String(), evt.Info.ID)
		if _, found := messageDedupCache.Get(dedupKey); found {
			log.Info().Str("key", dedupKey).Msg("WMIAU: Skipping duplicate message event")
			return
		}
		log.Debug().Str("key", dedupKey).Msg("WMIAU: Processing new message event")
		messageDedupCache.Set(dedupKey, true, cache.DefaultExpiration)
		...
		myuserinfo, found := userinfocache.Get(mycli.token)
		if !found {
			query := mycli.db.Rebind("SELECT CASE WHEN s3_enabled = 1 THEN 'true' ELSE 'false' END AS s3_enabled, media_delivery FROM users WHERE id = ?")
			err := mycli.db.Get(&s3Config, query, txtid)
			if err != nil {
		...
		go func() {
			...
			if err == nil && cwConfig.Enabled {
				...
				isStatus := evt.Info.Chat.Server == "broadcast" || evt.Info.Multicast
				if isStatus && !cwConfig.SendStatusStories {
					log.Debug().Msg("Skipping status story for Chatwoot (disabled in config)")
					return
				}

				svc := NewChatwootService(mycli.db, cwConfig, mycli.WAClient)

				err = svc.HandleIncomingWhatsAppMessage(mycli.WAClient, evt.Info.Chat, evt.Info.Sender, evt.Message, evt.Info.ID, evt.Info.IsGroup, evt.Info.IsFromMe, evt.Info.PushName, evt.Info.SenderAlt, evt.Info.RecipientAlt, evt.Info.Timestamp)
				if err != nil {
					log.Error().Err(err).Str("id", evt.Info.ID).Msg("Failed to forward message to Chatwoot")
				}
			}
		}()
		...
		if historyLimit > 0 {
			...
			if protocolMsg := evt.Message.GetProtocolMessage(); protocolMsg != nil && protocolMsg.GetType() == 0 {
				...
				// Chatwoot integration for delete is now handled natively in HandleIncomingWhatsAppMessage
				...
				// Chatwoot integration for reaction is now handled natively in HandleIncomingWhatsAppMessage
	...
	case *events.Receipt:
		...
		if evt.Type == types.ReceiptTypeRead {
			...
			postmap["state"] = "Read"
			go mycli.syncChatwootMessageReceiptStatus(evt.MessageIDs, "read")
		} else if evt.Type == types.ReceiptTypeReadSelf {
			log.Info().Strs("id", evt.MessageIDs).Str("source", evt.SourceString()).Str("timestamp", fmt.Sprintf("%v", evt.Timestamp)).Msg("Message was read by self")
			...
			go mycli.syncChatwootMessageReceiptStatus(evt.MessageIDs, "delivered")
		} else {
			...
			go func() {
				...
				for _, conv := range evt.Data.Conversations {
					...
					for _, msg := range conv.Messages {
						...
						// Fix: If message is from me and it is a group, ensure senderJID is set to my JID (accountOwnerJID)
						// The original logic above might have set it correctly, but let's be explicit because
						// different WhatsApp versions might behave differently or accountOwnerJID might be empty initially.
						if isFromMe && (chatJID.Server == types.GroupServer || chatJID.Server == types.BroadcastServer) {
							if accountOwnerJID != "" {
								senderJID = accountOwnerJID
							} else {
								// Fallback to "me" logic handled above, but ensure we don't accidentally set it to group JID
							}
						}
						...
						if message.GetConversation() != "" {
							...
							if contextInfo := audio.GetContextInfo(); contextInfo != nil {
								quotedMessageID = contextInfo.GetStanzaID()
							}
							messageType = "audio"
	...
	case *events.LoggedOut:
		...
		defer func() {
			signalKillChannel(mycli.userID)
		}()
		sqlStmt := mycli.db.Rebind(`UPDATE users SET connected=0 WHERE id=?`)
		_, err := mycli.db.Exec(sqlStmt, mycli.userID)
		if err != nil {
			log.Error().Err(err).Msg(sqlStmt)
			return
	...
	case *events.ChatPresence:
		...
		postmap["chat"] = evt.Chat.String()
		postmap["sender"] = evt.Sender.String()
		postmap["isFromMe"] = false
		if evt.State == types.ChatPresenceComposing {
			postmap["state"] = "composing"
		} else {
			postmap["state"] = "paused"
		}

		// Chatwoot Integration
		go func() {
			var cwConfig ChatwootConfig
			err := mycli.db.QueryRow(`
				SELECT user_id, enabled, account_id, token, url, inbox_id, inbox_name,
					   sign_msg, sign_delimiter, reopen_conversation, conversation_pending, merge_brazil_contacts
				FROM chatwoot_config WHERE user_id = $1
			`, txtid).Scan(
				&cwConfig.UserID, &cwConfig.Enabled, &cwConfig.AccountID, &cwConfig.Token, &cwConfig.URL,
				&cwConfig.InboxID, &cwConfig.InboxName, &cwConfig.SignMsg, &cwConfig.SignDelimiter,
				&cwConfig.ReopenConversation, &cwConfig.ConversationPending, &cwConfig.MergeBrazilContacts,
			...
			if err == nil && cwConfig.Enabled {
				...
				// Ignore group chats for presence/typing
				if strings.HasSuffix(evt.Chat.String(), "@g.us") {
					return
				}

				// We used to call svc.SendTypingEvent(evt.Chat.String(), status) here,
				// but it caused an echo loop where Chatwoot thought the Agent
				// was typing when the Customer was actually typing.
			}
		}()
	...
	case *events.LabelEdit:
		if evt.Action != nil {
			...
			log.Info().Str("labelID", evt.LabelID).Str("name", name).Msg("WhatsApp Label edited")
			query := mycli.db.Rebind(`
				INSERT INTO chatwoot_labels (user_id, label_id, name)
				VALUES (?, ?, ?)
				ON CONFLICT (user_id, label_id) DO UPDATE SET name = EXCLUDED.name
			`)
			// Note: ON CONFLICT syntax differs between SQLite and Postgres,
			// but modern SQLite supports the "post-postgres" syntax or we can use OR REPLACE.
			// Let's use a more compatible approach if needed, but Rebind helps with placeholders.
			if mycli.db.DriverName() == "sqlite" {
				query = `INSERT OR REPLACE INTO chatwoot_labels (user_id, label_id, name) VALUES (?, ?, ?)`
			}
			_, err := mycli.db.Exec(query, mycli.userID, evt.LabelID, name)
			if err != nil {
				log.Warn().Err(err).Msg("Failed to store label mapping")
			} else {
				// Success! Now check if there are any pending associations for this label to sync to Chatwoot
				go func(labelID, labelName string) {
					var chats []string
					err := mycli.db.Select(&chats, mycli.db.Rebind("SELECT remote_jid FROM chatwoot_conversation_labels WHERE user_id = ? AND label_id = ?"), mycli.userID, labelID)
					if err == nil && len(chats) > 0 {
						log.Info().Int("count", len(chats)).Str("label", labelName).Msg("Syncing pending label associations to Chatwoot")
						// Get Chatwoot config and sync each chat
						// (Reusing the logic from LabelAssociationChat but for multiple chats)
						var cwConfig ChatwootConfig
						err = mycli.db.Get(&cwConfig, mycli.db.Rebind("SELECT * FROM chatwoot_config WHERE user_id = ?"), mycli.userID)
						if err == nil && cwConfig.Enabled {
							svc := NewChatwootService(mycli.db, cwConfig, mycli.WAClient)
							for _, chatJID := range chats {
								parsedJID, _ := watypes.ParseJID(chatJID)
								_ = svc.SyncLabelToChatwoot(mycli.WAClient, parsedJID, labelName, "add")
							}
						}
					}
				}(evt.LabelID, name)
	...
	case *events.LabelAssociationChat:
		...
		log.Info().Str("labelID", evt.LabelID).Str("chat", evt.JID.String()).Str("action", action).Msg("WhatsApp Label association changed")

		go func() {
			// Persist association locally
			var labelQuery string
			if action == "add" {
				labelQuery = mycli.db.Rebind("INSERT INTO chatwoot_conversation_labels (user_id, remote_jid, label_id) VALUES (?, ?, ?) ON CONFLICT DO NOTHING")
				if mycli.db.DriverName() == "sqlite" {
					labelQuery = "INSERT OR IGNORE INTO chatwoot_conversation_labels (user_id, remote_jid, label_id) VALUES (?, ?, ?)"
				}
			} else {
				labelQuery = mycli.db.Rebind("DELETE FROM chatwoot_conversation_labels WHERE user_id = ? AND remote_jid = ? AND label_id = ?")
			}
			_, err := mycli.db.Exec(labelQuery, mycli.userID, evt.JID.String(), evt.LabelID)
			if err != nil {
				log.Warn().Err(err).Str("action", action).Msg("Failed to persist label association")
			}
			...
			query := mycli.db.Rebind(`
				...
				FROM chatwoot_config WHERE user_id = ?
			`)
			err = mycli.db.QueryRow(query, txtid).Scan(
				&cwConfig.UserID, &cwConfig.Enabled, &cwConfig.AccountID, &cwConfig.Token, &cwConfig.URL,
				&cwConfig.InboxID, &cwConfig.InboxName, &cwConfig.SignMsg, &cwConfig.SignDelimiter,
				&cwConfig.ReopenConversation, &cwConfig.ConversationPending, &cwConfig.MergeBrazilContacts,
				&cwConfig.ImportGroups, &cwConfig.SendStatusStories, &cwConfig.EnabledAt,
			...
			// Only sync to Chatwoot if this is a "live" event
			// (i.e., we've been connected for more than 30 seconds)
			if !mycli.ConnectTime.IsZero() && time.Since(mycli.ConnectTime) > 30*time.Second {
				var cwConfig ChatwootConfig
				query := mycli.db.Rebind(`
					SELECT user_id, enabled, account_id, token, url, inbox_id, inbox_name,
						   sign_msg, sign_delimiter, reopen_conversation, conversation_pending, merge_brazil_contacts,
				...
				if err == nil && cwConfig.Enabled {
					...
					// Get label name from DB
					var labelName string
					nameQuery := mycli.db.Rebind("SELECT name FROM chatwoot_labels WHERE user_id = ? AND label_id = ?")
					err := mycli.db.QueryRow(nameQuery, txtid, evt.LabelID).Scan(&labelName)
					if err != nil {
						log.Warn().Err(err).Str("labelID", evt.LabelID).Msg("Label name not found in DB for sync")
						return
					}

					// Sync to Chatwoot
					err = svc.SyncLabelToChatwoot(mycli.WAClient, evt.JID, labelName, action)
					if err != nil {
						log.Error().Err(err).Msg("Failed to sync label to Chatwoot")
					}
				}
			} else {
				log.Debug().Str("labelID", evt.LabelID).Str("chat", evt.JID.String()).Msg("Ignoring historic label association during startup")
			}
		}()
	default:
...
func (mycli *MyClient) fetchAndStoreLabels() {
	log.Debug().Str("userID", mycli.userID).Msg("Triggering AppState sync for labels")
	// Triggering a full sync of AppState (regular patch usually contains labels)
	// will cause whatsmeow to emit LabelEdit events for all existing labels.
	// We don't need a direct list if we handle the events robustly.
	// whatsmeow automatically does this on connection if EmitAppStateEventsOnFullSync is true,
	// but calling FetchAppState can force it if missing.

	// Available patch names: critical_block, critical_unblock_low, regular, regular_high, regular_low
	err := mycli.WAClient.FetchAppState(context.Background(), appstate.WAPatchRegular, true, false)
	if err != nil {
		log.Warn().Err(err).Msg("Failed to fetch app state for labels")
	}

	// Wait a bit and try regular_low as well just in case
	time.Sleep(2 * time.Second)
	_ = mycli.WAClient.FetchAppState(context.Background(), appstate.WAPatchRegularLow, true, false)
}
