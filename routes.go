...
func (s *server) routes() {
	...
	c = c.Append(hlog.AccessHandler(func(r *http.Request, status, size int, duration time.Duration) {
		hlog.FromRequest(r).Info().
			...
			Str("userid", func() string {
				if userValues, err := getUserInfo(r); err == nil {
					return userValues.Get("Id")
				}
				return ""
			}()).
			Msg("Got API Request")
	...
	s.router.Handle("/media/upload", c.Then(s.UploadMedia())).Methods("POST")
	s.router.Handle("/resolve/business", c.Then(s.ResolveBusinessMessageLink())).Methods("POST")
	s.router.Handle("/resolve/contact", c.Then(s.ResolveContactQRLink())).Methods("POST")
	...
	s.router.Handle("/user/devices", c.Then(s.GetUserDevices())).Methods("POST")

	// Privacy & Blocklist
	s.router.Handle("/privacy/settings", c.Then(s.GetPrivacySettings())).Methods("GET")
	s.router.Handle("/privacy/settings", c.Then(s.SetPrivacySetting())).Methods("POST")
	s.router.Handle("/blocklist", c.Then(s.GetBlocklist())).Methods("GET")
	s.router.Handle("/blocklist/update", c.Then(s.UpdateBlocklist())).Methods("POST")
	...
	s.router.Handle("/group/description", c.Then(s.SetGroupDescription())).Methods("POST")
	s.router.Handle("/group/requests", c.Then(s.GetGroupRequestParticipants())).Methods("POST")
	s.router.Handle("/group/requests/update", c.Then(s.UpdateGroupRequestParticipants())).Methods("POST")
	s.router.Handle("/group/join/link", c.Then(s.JoinGroupWithLink())).Methods("POST")
	s.router.Handle("/group/subgroups", c.Then(s.GetSubGroups())).Methods("POST")
	s.router.Handle("/group/link", c.Then(s.LinkGroup())).Methods("POST")
	s.router.Handle("/group/unlink", c.Then(s.UnlinkGroup())).Methods("POST")

	// Chatwoot integration routes
	s.router.Handle("/chatwoot/setup", c.Then(s.SetupChatwoot())).Methods("POST")
	s.router.Handle("/chatwoot/config", c.Then(s.GetChatwootConfig())).Methods("GET")
	s.router.Handle("/chatwoot/config", c.Then(s.SetupChatwoot())).Methods("POST")
	s.router.HandleFunc("/chatwoot/webhook/{instance}", s.ChatwootWebhook()).Methods("POST")

	s.router.Handle("/newsletter/list", c.Then(s.ListNewsletter())).Methods("GET")
	s.router.Handle("/newsletter/info", c.Then(s.GetNewsletterInfo())).Methods("POST")
	s.router.Handle("/newsletter/messages", c.Then(s.GetNewsletterMessages())).Methods("POST")
	...
	s.router.Handle("/newsletter/unmute", c.Then(s.UnmuteNewsletter())).Methods("POST")

	// Instance configuration endpoints
	s.router.Handle("/user/language/{id}", c.Then(s.UpdateLanguage())).Methods("PUT")

	// Redirect /login and /login/ to /dashboard/
	s.router.Handle("/login", http.RedirectHandler("/dashboard/", http.StatusFound))
	s.router.Handle("/login/", http.RedirectHandler("/dashboard/", http.StatusFound))
